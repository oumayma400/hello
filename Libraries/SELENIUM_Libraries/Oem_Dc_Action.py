...
def create_dc_action_Compaign(driver,dc_id, dc_action_id):
    time.sleep(2)
    ...
    step1 = WebDriverWait(driver, 10).until(EC.element_to_be_clickable((By.XPATH, Locator.dc_actionl_page)))
    step1.click()
    select_group = Select(driver.find_element_by_id("dcAction"))
    select_group.select_by_value(str(dc_action_id))
    name= 'AUTO_DC_ACTION_'+str(datetime.now().isoformat())+'_'+str(random.randint(1,100001))
